...
CustomContent.seed(:key,
  ...
  {key: Signup::AboMagazinMailer::CONFIRMATION,
   placeholders_required: "first-name, last-name, birthday, email, address-care-of, " \
     "street-with-number, postbox, zip-code, town, country, abo-name, language, gender, " \
     "costs, newsletter-subscribed, agb-link, data-protection-link"},
  {key: Signup::SektionMailer::CONFIRMATION,
   placeholders_required: "person-ids, first-name, last-name, birthday, email, phone-number, " \
     "address-care-of, street-with-number, postbox, zip-code, town, country, section-name, " \
     "membership-category, invoice-details",
   placeholders_optional: "profile-url, profile-links, faq-url"},
  ...
  {key: Invoices::SacMembershipsMailer::MEMBERSHIP_ACTIVATED,
   ...
   placeholders_optional: "profile-url, profile-links, person-ids, last-name, birthday, email, " \
     "phone-number, address-care-of, street-with-number, postbox, zip-code, town, country, " \
     "section-name, membership-category,  invoice-details,  faq-url"},
  {key: Memberships::TerminateMembershipMailer::LEAVE_ZUSATZSEKTION,
   placeholders_required: "person-name, sektion-name, terminate-on",
   placeholders_optional: "profile-links"},
  {key: Memberships::TerminateMembershipMailer::TERMINATE_MEMBERSHIP,
   placeholders_required: "person-name, sektion-name, terminate-on",
   placeholders_optional: "profile-links"},
  {key: Memberships::TerminateAboMagazinAbonnentMailer::TERMINATE_ABONNENT,
   placeholders_required: "person-name, terminate-on",
   placeholders_optional: "profile-links"},
  {key: Memberships::SwitchStammsektionMailer::CONFIRMATION,
   placeholders_required: "person-name, group-name",
   placeholders_optional: "person-id, profile-links"},
...
CustomContent::Translation.seed_once(:custom_content_id, :locale,
  ...
  {custom_content_id: CustomContent.get(Event::ApplicationConfirmationMailer::APPLIED).id,
   ...
   body: "Hallo {recipient-name},<br><br>" \
    "Du wurdest für den Kurs {event-name} (Nummer: {event-number}) auf die unbestätigte " \
    "Warteliste gesetzt. " \
  ...
  {custom_content_id: CustomContent.get(Event::LeaderReminderMailer::REMINDER_8_WEEKS).id,
   ...
   subject: "Erinnerung Kursstart",
   body: "Hallo {recipient-name},<br><br>" \
    "Der Kurs {event-name} (Nummer: {event-number}) findet 6 Wochen nach dem " \
  ...
  {custom_content_id: CustomContent.get(Event::PublishedMailer::NOTICE).id,
   ...
   body: "Hallo {recipient-name},<br><br>" \
    "Der Kurs {event-name} (Nummer: {event-number}), der 6 Wochen nach dem " \
    "{six-weeks-before-start} statt findet, wurde veröffentlicht. " \
    "Anmeldebeginn ist der {application-opening-at}." \
    "<br><br>" \
    "Event-Link: {event-link}<br>" \
    "Kursdetails:<br><br>{event-details}"},
  {custom_content_id: CustomContent.get(Event::ApplicationPausedMailer::NOTICE).id,
   ...
   body: "Lieber Kursadmin,<br><br>" \
    "Die Anmeldung für den Kurs {event-name} (Nummer: {event-number}) wurde pausiert." \
    "<br><br>" \
    "Event-Link: {event-link}<br>" \
    "Kursdetails:<br><br>{event-details}"},
  {custom_content_id: CustomContent.get(Event::ApplicationClosedMailer::NOTICE).id,
   ...
   body: "Lieber Kursadmin,<br><br>" \
    "Die Anmeldung für den Kurs {event-name} (Nummer: {event-number}) wurde abgeschlossen." \
    "<br><br>" \
    "Event-Link: {event-link}<br>" \
    "Kursdetails:<br><br>{event-details}"},
  ...
  {custom_content_id: CustomContent.get(Event::SurveyMailer::SURVEY).id,
   ...
   subject: "Kursumfrage",
   body: "Hallo {recipient-name},<br><br>" \
    "Wir hoffen, der Kurs {event-name} (Nummer: {event-number}) hat dir gut gefallen. " \
    "Es würde uns sehr freuen, wenn du dir einen Moment Zeit nehmen könntest, um an unserer " \
    "Umfrage teilzunehmen: {survey-link}.<br><br>" \
    "Anmeldung: {application-url}<br>" \
    "Person: {person-url}<br>" \
    "Event-Link: {event-link}<br>" \
    "Kursdetails:<br><br>{event-details}"},
  {custom_content_id: CustomContent.get(Qualifications::ExpirationMailer::REMINDER_TODAY).id,
   ...
   body: "Liebe(r) Tourenleiter(in), deine TL-Anerkennung ist ab morgen sistiert. " \
    ...
    "Bitte absolviere die nötigen Fortbildungstage, um wieder als aktive(r) Leiter(in) " \
    "registriert sein zu können."},
  ...
  {custom_content_id:
     CustomContent.get(Qualifications::ExpirationMailer::REMINDER_YEAR_AFTER_NEXT_YEAR).id,
   locale: "de",
  ...
  {custom_content_id: CustomContent.get(Signup::SektionMailer::CONFIRMATION).id,
   ...
   body: "Hallo {first-name},<br><br>" \
    ...
    "Ort: {town}<br><br>" \
    "Land: {country}<br><br>" \
    "Du wirst in kürze eine weitere E-Mail mit der Rechnung erhalten. Sobald die Zahlung bei " \
    "uns eingegangen ist, wird deine Mitgliedschaft vollständig aktiviert und alle Dienste " \
    "können genutzt werden.<br><br>" \
    "Den digitalen Mitgliederausweis findest du in deinem " \
    "<a href='{profile-url}'>SAC-Portal</a> Profil. " \
    "Weitere Details zum digitalen Mitgliederausweis findest im " \
    "<a href='{faq-url}'>FAQ</a>.<br><br>" \
    "Viel Spass beim SAC!"},
  ...
  {custom_content_id: CustomContent.get(Memberships::JoinZusatzsektionMailer::CONFIRMATION).id,
   ...
   body: "Hallo {first-name},<br><br>" \
    ...
    "Vielen Dank!"},
  {custom_content_id:
     CustomContent.get(Memberships::JoinZusatzsektionMailer::APPROVAL_PENDING_CONFIRMATION).id,
   locale: "de",
   ...
   body: "Hallo {first-name},<br><br>" \
    ...
    "Land: {country}<br><br>" \
    "Du wirst in kürze eine weitere E-Mail mit der Rechnung erhalten. Sobald die Zahlung bei " \
    "uns eingegangen ist, wird deine Zusatzmitgliedschaft vollständig aktiviert und alle " \
    "Dienste können genutzt werden.<br><br>" \
    "Den digitalen Mitgliederausweis findest du in deinem " \
    "<a href='{profile-url}'>SAC-Portal</a> Profil. " \
    "Weitere Details zum digitalen Mitgliederausweis findest im " \
    "<a href='{faq-url}'>FAQ</a>.<br><br>" \
  ...
  {custom_content_id: CustomContent.get(Invoices::SacMembershipsMailer::MEMBERSHIP_ACTIVATED).id,
   ...
   subject: "SAC Eintritt Bestellbestätigung",
   body: "Hallo {first-name},<br><br>" \
    "Vielen Dank für deine Zahlung, welche bei uns eingegangen ist. Deine Mitgliedschaft ist " \
    "jetzt vollständig aktiviert und alle Dienste des Schweizer Alpen-Clubs SAC können genutzt " \
    "werden.<br><br>" \
    "Deinen digitalen Mitgliederausweis findest du in deinem Profil im " \
    "<a href='{profile-url}'>SAC-Portal</a>. " \
    "Weitere Details zum digitalen Mitgliederausweis findest du in den FAQ.<br><br> " \
    "Viel Spass beim SAC!"},
  {custom_content_id:
     CustomContent.get(Memberships::TerminateMembershipMailer::LEAVE_ZUSATZSEKTION).id,
   locale: "de",
  ...
  {custom_content_id:
     CustomContent.get(Memberships::TerminateMembershipMailer::TERMINATE_MEMBERSHIP).id,
   locale: "de",
  ...
  {custom_content_id:
     CustomContent.get(Memberships::TerminateAboMagazinAbonnentMailer::TERMINATE_ABONNENT).id,
   locale: "de",
   label: "Mitgliedschaften: Kündigung Die Alpen",
   subject: "Kündigung Die Alpen",
   body: "Hallo {person-name},<br><br>" \
    "Die Kündigung der Die Alpen wurde per {terminate-on} vorgenommen."},
  ...
  {custom_content_id: CustomContent.get(People::NeuanmeldungenMailer::APPROVED).id,
   ...
   body: "Hallo {first-name},<br><br>" \
    "Vielen Dank für dein Interesse an einer Mitgliedschaft beim Schweizer Alpen-Club SAC." \
    "<br><br>" \
    "Die {sektion-name} hat deinen Antrag geprüft und wir freuen uns, dir mitzuteilen, dass " \
    "dein Antrag angenommen wurde. Herzlich Willkommen beim Schweizer Alpen-Club SAC!<br><br>" \
    "Du wirst in Kürze eine weitere E-Mail mit der Mitglieder-Rechnung erhalten. Sobald die " \
    "Zahlung bei uns eingegangen ist, wird deine Mitgliedschaft vollständig aktiviert und alle " \
    "Dienste können genutzt werden.<br><br>" \
    "Den digitalen Mitgliederausweis findest du in deinem Profil im " \
    "<a href='{profile-url}'>SAC-Portal</a>. " \
    "Weitere Details zum digitalen Mitgliederausweis findest du in den FAQ.<br><br>" \
    "Viel Spass beim SAC!"},
