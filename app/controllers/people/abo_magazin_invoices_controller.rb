...
class People::AboMagazinInvoicesController < CrudController
  ...
  def create
    if valid_issued_at?
      if person.data_quality != "error"
        ...
        entry.year = role_of_selected_magazin.end_on&.year
        super.then do
  ...
  def valid_issued_at?
    end_on = role_of_selected_magazin&.end_on
    permitted_params[:issued_at].to_date == end_on&.next_day || end_on.nil? && permitted_params[:issued_at].nil?
  end
  ...
  def role_of_selected_magazin = @role_of_selected_magazin ||= Role.find_by(person_id: person.id, group_id: permitted_params[:link_id])
