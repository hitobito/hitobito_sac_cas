...
module Invoices
  module SacMemberships
    ...
    class Member
      ...
      delegate :stammsektion, :stammsektion_role, :zusatzsektion_roles,
        :neuanmeldung_nv_stammsektion_roles, :neuanmeldung_nv_zusatzsektion_roles,
        :sac_ehrenmitglied?, :sektion_ehrenmitglied?, :sektion_beguenstigt?,
        to: :sac_membership

      # Person model
      def initialize(person, context)
        @person = person
        @sac_membership = People::SacMembership.new(person, context.date)
        @context = context
      end
