  ...
  module SacMemberships
    module Positions
      class SectionBulletinPostageAbroad < Base
        ...
        def section_bulletin?
          section_bulletin_mailing_list &&
            !section_bulletin_mailing_list&.subscriptions&.exists?(
              subscriber: member.person,
              excluded: true
            )
        end

        def section_bulletin_mailing_list
          MailingList.find_by(
