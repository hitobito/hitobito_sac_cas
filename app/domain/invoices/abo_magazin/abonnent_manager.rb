...
class Invoices::AboMagazin::AbonnentManager
  ...
  def update_abonnent_status
    ActiveRecord::Base.transaction do
      if abonnent_role
        ...
        create_new_abonnent_role(expired_neuanmeldung_abonnent_role, expired_neuanmeldung_abonnent_role.start_on)
      end
    end
  end
  ...
  def abonnent_role = @abonnent_role ||= person.roles.where(type: ABONNENT_ROLE_TYPE.sti_name).first

  def expired_abonnent_role = @expired_abonnent_role ||= person.roles.with_inactive.where(type: ABONNENT_ROLE_TYPE.sti_name, end_on: 1.year.ago..Time.zone.today).first

  def neuanmeldung_abonnent_role = @neuanmeldung_abonnent_role ||= person.roles.where(type: NEUANMELDUNG_ABONNENT_ROLE_TYPE.sti_name).first

  def expired_neuanmeldung_abonnent_role = @expired_neuanmeldung_abonnent_role ||= person.roles.with_inactive.where(type: NEUANMELDUNG_ABONNENT_ROLE_TYPE.sti_name).first
