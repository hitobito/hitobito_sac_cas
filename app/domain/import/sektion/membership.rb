  ...
  module Sektion
    class Membership
      ...
      def assign_household(household_key)
        return unless household_key.present?
      ...
      def remove_placeholder_contact_role
        Group::ExterneKontakte::Kontakt.where(person: person, group: placeholder_contact_group).delete_all
      end
      ...
      def joining_date
        date = last_joining_date
        date == unknown_joining_date ? joining_year_date : date
      end

      def last_joining_date
        parse_date(row[:last_joining_date]) or
          raise "Invalid 'Letztes Eintrittsdatum' for #{navision_id}: #{row[:last_joining_date]}"
      end

      def unknown_joining_date
        Date.new(1900, 1, 1)
      end

      def joining_year_date
        row[:joining_year].present? && row[:joining_year].is_a?(Integer) or
          raise "Invalid 'Eintrittsjahr' for #{navision_id}: #{row[:joining_year]}"

        Date.new(row[:joining_year])
      end

      def last_exit_date
        parse_date(row[:last_exit_date]) or
          raise "Invalid 'Letztes Austrittsdatum' for #{navision_id}: #{row[:last_exit_date]}"
      end
