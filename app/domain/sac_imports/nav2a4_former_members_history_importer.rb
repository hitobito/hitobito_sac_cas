...
module SacImports
  class Nav2a4FormerMembersHistoryImporter < Nav2Base
    self.source_file = :NAV2a
    self.report_name = "nav2a1-roles-membership"

    WHODUNNIT = "import script HIT-1002"

    private

    def run_import
      PaperTrail.request.whodunnit = WHODUNNIT
      @people_with_memberships = Role.unscoped.where(type: SacCas::MITGLIED_ROLES.map(&:sti_name))
        .select(:person_id).distinct.pluck(:person_id)
