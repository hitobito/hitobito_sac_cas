...
class Invoices::Abacus::CreateInvoiceJob < BaseJob
  ...
  def perform
    if membership_invoice.invoice?
      ...
      create_error_log_entry(I18n.t(ExternalInvoice::SacMembership::NO_MEMBERSHIPS_KEY))
    else
      external_invoice.update!(state: :error)
      create_error_log_entry(I18n.t(ExternalInvoice::SacMembership::NOT_POSSIBLE_KEY))
    end
  end
