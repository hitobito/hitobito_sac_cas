...
class Export::AlpsRecipientsExportJob < Export::ExportBaseJob
  ...
  def initialize(user_id, filename, reference_date, new_entries_from, **)
    ...
    super(:zip, user_id, filename: filename, **)
  end
  ...
  def generate_new_entries(lang)
    write_temp_xlsx(recipients_scope.new_entries(lang), lang:)
  end

  def write_temp_xlsx(scope, full: false, lang: nil)
    Tempfile.new(["alps_recipients", ".xlsx"]).tap do |tmpfile|
      tmpfile.binmode
      tmpfile.write(generate_xlsx(scope, full: full, lang: lang))
      tmpfile.flush
    end
  end

  def generate_xlsx(scope, full: false, lang: nil)
