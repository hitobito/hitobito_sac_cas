...
describe Events::Filter::Leader do
  ...
  let(:kurs1) { Fabricate(:sac_course) }
  let(:kurs2) { Fabricate(:sac_course) }
  let(:kurs3) { Fabricate(:sac_course) }
  let(:kurs4) { Fabricate(:sac_course) }

  let!(:kurs1_leader) do
    p1 = Fabricate(:event_participation, event: kurs1)
    Event::Course::Role::Leader.create!(participation: p1)
    p2 = Fabricate(:event_participation, event: kurs4, participant: p1.person)
    Event::Course::Role::Participant.create!(participation: p2)
  ...
  let!(:kurs2_leader) do
    ...
    Event::Course::Role::Leader.create!(participation: p)
    p.person
  end
