...
describe SearchStrategies::SqlConditionBuilder do
  ...
  describe "birthday" do
    ...
    it "uses custom matcher for birthday" do
      expect(build("1.10.2014", %w[people.birthday]))
        .to eq "TO_CHAR(\"people\".\"birthday\", 'DD.MM.YYYY') ILIKE '%01.10.2014%'"
      expect(build("01.10.2014", %w[people.birthday]))
        .to eq "TO_CHAR(\"people\".\"birthday\", 'DD.MM.YYYY') ILIKE '%01.10.2014%'"
      expect(build("01.10", %w[people.birthday]))
        .to eq "TO_CHAR(\"people\".\"birthday\", 'DD.MM.YYYY') ILIKE '%01.10%'"
      expect(build("2014", %w[people.birthday]))
        .to eq "TO_CHAR(\"people\".\"birthday\", 'DD.MM.YYYY') ILIKE '%2014%'"
    end
