...
describe Export::Xlsx::SacStatistics::SektionMitgliederAndZusatzmitgliederSheet do
  ...
  it "renders xlsx" do
    ...
    expect(numbers(:bluemlisalp)).to eq(["SAC Blüemlisalp", "Sektion", 4, 1, 0, 1, 1, 1])
    expect(numbers(:matterhorn)).to eq(["SAC Matterhorn", "Sektion", 4, 1, 0, 1, 1, 1])

    expect(xlsx.rows.last).to eq(
      ["Total", nil, nil, 8, 2, 0, 2, 2, 2]
    )
  end
end
