...
describe SacImports::QualificationsImporter do
  ...
  let(:report_headers) {
    %w[navision_id hitobito_person navision_qualification_active
      navision_start_at navision_finish_at navision_qualification_kind errors]
  }
  ...
  it "creates report for members in source file" do
    ...
    expect { report.create }
      .to change { Qualification.count }.by(23)

    expect(Qualification.last.attributes).to include(
      "person_id" => 4200003,
      "qualification_kind_id" => QualificationKind.find_by(label: "SAC Tourenleiter*in 1 Sommer").id,
      "start_at" => Date.new(2020, 7, 3),
      "finish_at" => Date.new(2028, 12, 31)
    )
