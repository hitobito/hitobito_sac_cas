...
describe Import::PeopleImporter do
  ...
  it 'imports natürliche person' do
    ...
    expect(person.birthday).to eq(DateTime.new(2000, 10, 01))
  ...
  it 'imports juristische person' do
    importer.import!

    person = Person.find(people_navision_ids.third)

    expect(person.first_name).to eq('Vorname 3')
    expect(person.last_name).to eq('Nachname 3')
