...
describe Import::PersonEntry do
  ...
  describe 'validations' do
    ...
    it 'is valid with birthday less than 6 years ago' do
      row[:birthday] = (5.years + 11.month).ago
      expect(entry).to be_valid
      expect(entry.errors).to be_empty
    end
  ...
  describe 'person attributes' do
    ...
    it 'sets company_name for company row' do
      row[:name] = 'Firma 1'
      allow_any_instance_of(described_class).to receive(:company?).and_return(true)
      expect(person.company).to eq true
      expect(person.company_name).to eq 'Firma 1'
    end

    it 'detects company from person_type' do
      row[:person_type] = 'juristisch'
      expect(person.company).to eq true
    end

    it 'detects company from salutation' do
      row[:salutation] = 'Firma'
      expect(person.company).to eq true
    end
