  ...
  describe "abo magazin invoices" do
    ...
    context "no issues" do
      ...
      it "creates invoice for selected abo group" do
        ...
        Fabricate(Group::AboMagazin::Abonnent.sti_name, group: abo_group_fr, person: person, end_on: person.roles.first.end_on)
      ...
      it "updates issued_at from end_on + 1.day of abonnent role to start_on of neuanmeldungs role when selecting neuanmeldung" do
        # create second abo role for french magazine
        abo_group_fr = Fabricate(Group::AboMagazin.sti_name, parent: groups(:abo_magazine), name: "Les Alpes FR")
        neuanmeldung_role = Fabricate(Group::AboMagazin::Neuanmeldung.sti_name, group: abo_group_fr, person: person, start_on: Date.current)

        visit new_group_person_abo_magazin_invoice_path(group_id: group, person_id: person.id)
        expect(page).to have_text "Die Alpen Rechnung erstellen"
        expect(page).to have_field("Rechnungsdatum", with: I18n.l(roles(:abonnent_alpen).end_on.next_day), disabled: true)
        choose "Les Alpes FR", allow_label_click: true
        expect(page).to have_field("Rechnungsdatum", with: I18n.l(neuanmeldung_role.start_on), disabled: true)
        expect(page).not_to have_field("Rechnungsdatum", with: I18n.l(roles(:abonnent_alpen).end_on.next_day), disabled: true)
      end
    end
