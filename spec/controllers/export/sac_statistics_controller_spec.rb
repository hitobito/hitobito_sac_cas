# frozen_string_literal: true

#  Copyright (c) 2024, Schweizer Alpen-Club. This file is part of
#  hitobito_sac_cas and licensed under the Affero General Public License version 3
#  or later. See the COPYING file at the top-level directory or at
#  https://github.com/hitobito/hitobito_sac_cas.

require "spec_helper"

describe Export::SacStatisticsController do
  let(:user) { people(:admin) }
  let(:group) { groups(:root) }

  before { sign_in(user) }

  describe "POST #create" do
    it "starts export job when form is valid" do
      jobs = Delayed::Job.where("handler like '%SacStatisticsExportJob%'")
      expect do
        post :create,
          params: {
            group_id: group,
            people_export_period_form: {
              from: "1.4.2015", to: "31.12.2015"
            }
          }
      end.to change { jobs.count }.by(1)

      job = jobs.first
      expect(job.handler).to match("from: 2015-04-01")
      expect(job.handler).to match("to: 2015-12-31")
      expect(job.handler).to match("filename: SAC_Statistik_20150401_20151231-")
    end

    it "returns turbo frame when form is invalid" do
      post :create,
        params: {
          group_id: group,
          people_export_period_form: {
            from: "",
            to: ""
          }
        }
      expect(response.media_type).to eq Mime[:turbo_stream]
      expect(response.status).to eq(422)
    end
  end
end
