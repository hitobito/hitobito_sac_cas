      ...
      context "PUT#summon" do
        ...
        it "sends summon email when send_email is true" do
          expect { put :summon, params: params.merge(send_email: true) }
            .to have_enqueued_mail(Event::CourseParticipationMailer, :summon).once
        end

        it "does not send summon email when send_email is false" do
          expect { put :summon, params: params.merge(send_email: false) }
            .not_to have_enqueued_mail(Event::CourseParticipationMailer, :summon)
        end
      end
        ...
        context "as participant" do
          ...
          it "always sends email" do
            expect { put :cancel, params: params }
              .to have_enqueued_mail(Event::CourseParticipationMailer, :canceled).once
          end
        ...
        context "as admin" do
          ...
          it "sends application canceled email when send_email is true" do
            expect do
              put :cancel,
                params: params.merge({
                  event_participation: {canceled_at: 1.day.ago.to_date},
                  send_email: true
                })
  ...
  context "tour" do
    ...
    context "POST#create" do
      ...
      context "pricing" do
        ...
        context "member" do
          ...
          it "sets special price with stammsektion role in tour group" do
            expect do
              post :create, params:
            end.to change { Event::Participation.count }.by(1)
            expect(participation.price).to eq 50
            expect(participation.price_category).to eq "price_special"
          end

          it "sets special price with zusatzsektion role in tour group" do
          ...
          it "sets member price with active sac membership in other section" do
            ...
            expect(participation.price).to eq 60
            expect(participation.price_category).to eq "price_member"
          end
        ...
        context "non-member" do
          ...
          it "sets regular price without active membership" do
            expect do
              post :create, params:
            end.to change { Event::Participation.count }.by(1)
            expect(participation.price).to eq 80
            expect(participation.price_category).to eq "price_regular"
          end
          ...
          it "cannot set price or category itself" do
            expect do
              post :create,
                params: params.merge(event_participation: {price_category: "price_special", price: 1})
            end.to change { Event::Participation.count }.by(1)
            expect(participation.price).to eq 80
            expect(participation.price_category).to eq "price_regular"
          end
    ...
    context "state changes" do
      ...
      context "PUT#cancel" do
        context "as participant" do
          ...
          it "cancels participations, generates no invoice" do
            ...
            expect do
              expect do
                put :cancel, params: params
              end.to have_enqueued_mail(Event::TourParticipationMailer, :canceled).once
            end
        ...
        context "as admin" do
          let(:event) { Fabricate(:sac_tour, groups: [groups(:bluemlisalp)]) }
          ...
          it "cancels participations, generates no invoice but sends email" do
            ...
            expect do
              expect do
                put :cancel,
                  params: params.merge({
                    event_participation: {canceled_at: 5.days.ago.to_date},
                    send_email: false,
                    invoice_option: "standard"
                  })
              end.not_to have_enqueued_mail(Event::CourseParticipationMailer, :canceled)
