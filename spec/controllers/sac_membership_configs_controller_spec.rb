...
describe SacMembershipConfigsController do
  ...
  context "GET index" do
    it "redirects to latest config edit" do
      get :index, params: {group_id: Group.root_id}

      expect(response)
        .to redirect_to(edit_group_sac_membership_config_path(group_id: Group.root_id, id: latest_config.id))
    end
    ...
    it "redirects to new if no previous config present" do
      ...
      expect(response)
        .to redirect_to(new_group_sac_membership_config_path(group_id: Group.root_id))
    end
  ...
  context "GET show" do
    it "redirects to config edit" do
      get :show, params: {group_id: Group.root_id, id: older_config.id}

      expect(response)
        .to redirect_to(edit_group_sac_membership_config_path(group_id: Group.root_id, id: older_config.id))
    end
  ...
  context "GET edit" do
    it "renders config edit form" do
      get :edit, params: {group_id: Group.root_id, id: latest_config.id}
    ...
    it "cannot be accessed by non admin" do
      ...
      expect do
        get :show, params: {group_id: Group.root_id, id: latest_config.id}
      end.to raise_error(CanCan::AccessDenied)
    end
  end
  ...
  context "PATCH update" do
    ...
    it "updates config and redirects to show/edit form" do
      patch :update, params: {group_id: Group.root_id, id: latest_config.id}.merge(updated_model_params)

      expect(response)
        .to redirect_to(group_sac_membership_config_path(group_id: Group.root_id, id: latest_config.id))
  ...
  context "DELETE destroy" do
    it "cannot be destroyed" do
      expect do
        delete :destroy, params: {group_id: Group.root_id, id: older_config.id}
