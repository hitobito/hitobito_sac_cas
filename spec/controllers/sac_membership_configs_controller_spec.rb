...
describe SacMembershipConfigsController do
  ...
  context "GET index" do
    it "redirects to latest config edit" do
      ...
      expect(response).to redirect_to(
        edit_group_sac_membership_config_path(group_id: Group.root_id, id: latest_config.id)
      )
    end
    ...
    it "redirects to new if no previous config present" do
      ...
      expect(response).to redirect_to(
        new_group_sac_membership_config_path(group_id: Group.root_id)
      )
    end
  ...
  context "GET show" do
    it "redirects to config edit" do
      ...
      expect(response).to redirect_to(
        edit_group_sac_membership_config_path(group_id: Group.root_id, id: older_config.id)
      )
    end
  ...
  context "PATCH update" do
    ...
    it "updates config and redirects to show/edit form" do
      patch :update,
        params: {group_id: Group.root_id, id: latest_config.id}.merge(updated_model_params)

      expect(response).to redirect_to(
        group_sac_membership_config_path(group_id: Group.root_id, id: latest_config.id)
      )
