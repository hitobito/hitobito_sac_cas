...
describe People::Export::AustritteController do
  ...
  describe "POST #create" do
    it "starts export job when form is valid" do
      jobs = Delayed::Job.where("handler like '%AustritteExportJob%'")
      expect do
        post :create,
          params: {
            group_id: group,
      ...
      expect(job.handler).to match("filename: 578575972_SAC Blüemlisalp_Austritte_20150101_20151231-")
      expect(UserJobResult.find_by!(delayed_job: job).filename)
        .to match("578575972_SAC-Blueemlisalp_Austritte_20150101_20151231-")
    end
