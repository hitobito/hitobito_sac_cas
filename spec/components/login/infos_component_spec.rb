...
describe Login::InfosComponent, type: :component do
  ...
  describe "general" do
    ...
    it "renders title" do
      expect(body).to have_css("h2.card-title", text: "Fragen zum Login?")
    end

    it "info text with link to faqs" do
      expect(body).to have_css("p.card-text", text: "Mehr Informationen findest du unter den FAQs")
      expect(body).to have_link("FAQs", href: "https://www.sac-cas.ch/de/meta/faq/faq-sac-konto-/-sac-portal/")
    end
  end
  ...
  describe "contact" do
    ...
    [:de, :fr, :it].each do |locale|
      it "renders mv email in #{locale}" do
