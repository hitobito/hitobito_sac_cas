...
describe "events/_form_tab_pane_prices.html.haml" do
  ...
  context "tour" do
    ...
    it "renders price category headers" do
      expect(dom).to have_css "legend", text: "Kosten SAC Sektionsmitglied"
      expect(dom).to have_css "legend", text: "Kosten SAC-Mitglied (extern)"
      expect(dom).to have_css "legend", text: "Kosten nicht-SAC-Mitglied (Gast)"
    end
