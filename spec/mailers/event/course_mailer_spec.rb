...
describe Event::CourseMailer do
  let(:event) do
    Fabricate(:sac_open_course).tap { |c| c.update_attribute(:state, :assignment_closed) }
  end
  let(:mail) { described_class.application_closed(event) }
