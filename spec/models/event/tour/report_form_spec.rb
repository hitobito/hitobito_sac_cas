...
describe Event::Tour::ReportForm do
  ...
  describe "#release_recipients" do
    ...
    it "includes people with Tourenchef role in the tour's section" do
      ...
      expect(form.release_recipients).to include(recipient)
    end
    ...
    it "excludes people from a different section" do
      ...
      expect(form.release_recipients).not_to include(recipient)
    end
  end

  describe "#assert_recipient_present_when_forwarding" do
    ...
    it "is invalid when forwarding from draft without recipient" do
      ...
      form.recipient_id = nil

      expect(form).not_to be_valid
      expect(form.errors[:recipient_id]).to include("muss ausgefüllt werden")
    end
  ...
  describe "#save with status_action" do
    it "sets submitted_at and submitter_id when forwarding from draft" do
      form.status_action = "forward"
      form.recipient_id = recipient.id

      expect { form.save }
        .to change { report.reload.submitted_at }.from(nil)
        .and change { report.reload.submitter_id }.to(recipient.id)
    end
    ...
    it "sets approved_at and approver_id when forwarding from review" do
      ...
      expect { form.save }
        ...
        .and change { report.reload.approver_id }.to(recipient.id)
    end
