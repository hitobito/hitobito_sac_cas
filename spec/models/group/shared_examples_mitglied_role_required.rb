...
shared_examples "Mitglied role required" do
  ...
  let(:role) {
    described_class.new(person:, group:, start_on: 1.day.from_now, end_on: 4.days.from_now)
  }
  ...
  it "is invalid with MitgliedZusatzsektion role in different group" do
    Fabricate(Group::SektionsMitglieder::Mitglied.sti_name,
      ...
      group: groups(:bluemlisalp_ortsgruppe_ausserberg_mitglieder), person: person, end_on: end_of_next_year)
    # rubocop:enable Layout/LineLength
    Fabricate(Group::SektionsMitglieder::MitgliedZusatzsektion.sti_name, group: other_group,
      person: person, end_on: end_of_next_year)
  ...
  it "is valid with MitgliedZusatzsektion role in group" do
    Fabricate(Group::SektionsMitglieder::Mitglied.sti_name, group: other_group, person: person,
      end_on: end_of_next_year)
    Fabricate(Group::SektionsMitglieder::MitgliedZusatzsektion.sti_name, group: group,
      person: person, end_on: end_of_next_year)
