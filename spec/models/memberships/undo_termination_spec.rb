...
describe Memberships::UndoTermination, versioning: true do
  ...
  describe "#updated_role_versions" do
    it "returns all updated versions of roles with matching mutation_id" do
      ...
      expect(subject.updated_role_versions).to all have_attributes(mutation_id: "relevant mutations")
      expect(subject.updated_role_versions.map(&:reify)).to match_array [
    ...
    it "returns an empty array if mutation_id is nil" do
      ...
      expect(subject.updated_role_versions).to be_empty
    end
    ...
    it "returns the first version with matching mutation_id when multiple exist" do
      mutation("multiple versions with same mutation id") do
        ...
        expect(subject.updated_role_versions).to eq [expected_version]
      end
    end
  end
  ...
  describe "#created_role_versions" do
    it "returns all versions of created roles with matching mutation_id" do
      ...
      expect(subject.created_role_versions).to have(2).items
      expect(subject.created_role_versions.map { _1.item&.class&.sti_name })
        .to contain_exactly(
